...
public class WorkCenterServiceImpl implements WorkCenterService {
    ...
    private UnitMaster getUnitMaster(int unitId)  {
        return unitRepository.findByUnitIdAndIsActive(unitId,"Y")
                .orElseThrow(()  ->new WorkCenterException("UnitId Not Found",HttpStatus.NOT_FOUND));
    }
    ...
    public WorkCenterResponse updateWorkCenter(WorkCenterRequest workCenterRequest)  {
        ...
        if(workCenterMasterOptional.isEmpty()){
            log.error("Work-Center does not exists with the given workCenterName");
            throw  new WorkCenterException("Work-Center does not exists with the given workCenterName", HttpStatus.NOT_FOUND);
        }
    ...
    public WorkCenterResponse getWorkCenter(int workCenterId) {
        WorkCenterMaster workCenterMaster = workCenterRepository.findById(workCenterId)
    ...
    public WorkCenterResponse deleteWorkCenter(int workCenterId)  {
        WorkCenterMaster workCenterMaster = workCenterRepository.findById(workCenterId)
                .orElseThrow(() ->new WorkCenterException("WorkCenter does not exists with the given workCenterName",HttpStatus.NOT_FOUND));
        log.info("Query to fetch the WorkCenterMaster based on workCenterId");
